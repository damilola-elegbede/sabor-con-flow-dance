...
async function optimizeImage(inputPath, outputDir) {
  try {
    ...
    if (inputSize > config.performance.maxImageSize) {
      logger.warn(
        `Skipping large image: ${path.basename(inputPath)} (${(inputSize / 1024 / 1024).toFixed(2)}MB)`
      );
    ...
    for (const width of config.responsive.breakpoints) {
      ...
      for (const format of config.responsive.formats) {
        ...
        const _outputPath = path.join(outputDir, outputName);
...
async function optimizeImages() {
  ...
  for (const batch of batches) {
    const batchPromises = batch.map(imagePath => optimizeImage(imagePath, config.output.images));
    ...
    for (const result of batchResults) {
      ...
      logger.info(
        `Optimized: ${path.basename(result.inputPath)} -> ${result.results.length} variants (${(compressionRatio * 100).toFixed(1)}% reduction)`
      );
    }
  }
  ...
  logger.info(
    `  Space saved: ${(totalSaved / 1024 / 1024).toFixed(2)}MB (${(overallCompressionRatio * 100).toFixed(1)}%)`
  );
...
async function optimizeSVGs() {
  ...
  const svgPaths = await globby([path.join(config.input.svg, '**/*.svg')]);
  ...
  for (const svgPath of svgPaths) {
    try {
      ...
      logger.info(
        `Optimized SVG: ${baseName}.svg (${(compressionRatio * 100).toFixed(1)}% reduction)`
      );
    } catch (error) {
  ...
  logger.info(
    `  Space saved: ${(totalSaved / 1024).toFixed(2)}KB (${(overallCompressionRatio * 100).toFixed(1)}%)`
  );
...
async function optimizeFonts() {
  ...
  const fontPaths = await globby([path.join(config.input.fonts, '**/*.{woff,woff2,ttf,otf}')]);
...
async function main() {
  ...
  const optimizationType =
    process.argv.find(arg => arg.startsWith('--type='))?.split('=')[1] || 'all';
  ...
  try {
    ...
    Object.entries(results).forEach(([type, result]) => {
      if (result.totalSaved) {
        ...
        logger.info(
          `${type}: ${result.processed} files, ${(result.totalSaved / 1024 / 1024).toFixed(2)}MB saved`
        );
      } else {

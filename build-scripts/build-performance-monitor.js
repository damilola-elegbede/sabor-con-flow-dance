...
async function analyzeBundleSizes() {
  ...
  for (const assetFile of assetFiles.slice(0, 20)) { // Limit to first 20 for performance
...
function checkBuildTimePerformance(buildTimes) {
  ...
  Object.entries(buildTimes).forEach(([step, timing]) => {
    if (step !== 'total' && timing.duration > 10000) { // 10 second threshold for individual steps
      issues.push({
        type: 'build_time',
        severity: 'warning',
...
function compareMetrics(current, previous) {
  ...
  const currentTotalSize = current.bundleAnalysis.js.totalGzipped + current.bundleAnalysis.css.totalGzipped;
  const previousTotalSize = previous.bundleAnalysis.js.totalGzipped + previous.bundleAnalysis.css.totalGzipped;
...
async function generatePerformanceReport(metrics, comparison) {
  ...
  const report = {
    ...
    summary: {
      ...
      bundleSizeWithinTarget: (metrics.bundleAnalysis.js.totalGzipped + metrics.bundleAnalysis.css.totalGzipped) <= performanceTargets.bundleSize.total,
      totalIssues: metrics.performanceChecks.bundleSize.issues.length + metrics.performanceChecks.buildTime.issues.length,
...
<body>
    ...
    <div class="issues">
        ...
        ${report.performanceChecks.bundleSize.issues.map(issue => `
            <div class="issue issue-error">
                <strong>${issue.type}:</strong> ${issue.message}
            </div>
        `).join('')}
        ${report.performanceChecks.buildTime.issues.map(issue => `
            <div class="issue issue-error">
                <strong>${issue.type}:</strong> ${issue.message}
            </div>
        `).join('')}
    ...
    <div class="issues">
        ...
        ${report.performanceChecks.bundleSize.warnings.map(warning => `
    ...
    <div class="issues">
        ...
        ${report.comparison.regressions.map(regression => `
            <div class="issue issue-error">
    ...
    <div class="issues">
        ...
        ${report.comparison.improvements.map(improvement => `
...
async function main() {
  ...
  try {
    ...
    const hasIssues = bundleSizeChecks.issues.length > 0 || buildTimeChecks.issues.length > 0 || comparison.regressions.length > 0;
    ...
    logger.info(`  Total: ${((bundleAnalysis.js.totalGzipped + bundleAnalysis.css.totalGzipped) / 1024).toFixed(2)}KB`);
